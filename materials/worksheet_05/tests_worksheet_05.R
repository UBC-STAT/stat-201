...
test_2.0 <- function() {
  ...
  test_that('Did not assign answer to an object called "answer2.0_std_error"', {
...
test_4.1 <- function() {
  ...
  test_that("Data frame does not contain the correct data", {
    expect_equal(digest(as.integer(sum(penguins_diff_means_ci_t$lower_ci) * 1000)), "e9596bf996cb49d14bb9cac04896d35b")
    expect_equal(digest(as.integer(sum(penguins_diff_means_ci_t$upper_ci) * 1000)), "712e7c2007383713ed2e647b23ac7aae")
  })
...
test_5.1 <- function() {
  ...
  # Test 1: Check if the answer object exists
  test_that('Did not assign answer to an object called "ci_answer5.1"', {
    expect_true(exists("ci_answer5.1"))
  })

  # Test 2: Check if the object is a data frame (or tibble)
  test_that("Solution should be a data frame", {
    expect_true("data.frame" %in% class(ci_answer5.1))
  })

  # Test 3: Check for the correct column names
  expected_colnames <- c("lower_ci", "upper_ci")
  given_colnames <- colnames(ci_answer5.1)
  test_that("Data frame does not have the correct columns: 'lower_ci' and 'upper_ci'", {
    # This checks that there are no extra columns and no missing columns
    expect_equal(length(setdiff(
      union(expected_colnames, given_colnames),
      intersect(expected_colnames, given_colnames)
    )), 0)
  ...
  # Test 4: Check for the correct number of rows (should be 1)
  test_that("Data frame does not contain the correct number of rows (should be 1)", {
    # The digest for the integer 1 is '4b5630ee914e848e8d07221556b0a2fb'
    expect_equal(digest(as.integer(nrow(ci_answer5.1))), "4b5630ee914e848e8d07221556b0a2fb")
  })

  # Test 5: Check if the data values are correct using the provided digests
  test_that("The confidence interval values are not correct.", {
    # Check the lower bound
    expect_equal(digest(as.integer(ci_answer5.1$lower_ci * 100000)), 
                 'f18e3d47c6b8bda3b609c6df5ae666c4', 
                 info = "The lower_ci value is incorrect.")
    
    # Check the upper bound
    expect_equal(digest(as.integer(ci_answer5.1$upper_ci * 100000)), 
                 '957647922e1b4500c1f7da43ad022a71', 
                 info = "The upper_ci value is incorrect.")
  })
...
test_5.2 <- function() {
  ...
  # Test 1: Check if the answer object exists
  test_that('Did not assign answer to an object called "ci_answer5.2"', {
    expect_true(exists("ci_answer5.2"))
  })

  # Test 2: Check if the object is a data frame (or tibble)
  test_that("Solution should be a data frame", {
    expect_true("data.frame" %in% class(ci_answer5.2))
  })

  # Test 3: Check for the correct column names
  expected_colnames <- c("lower_ci", "upper_ci")
  given_colnames <- colnames(ci_answer5.2)
  test_that("Data frame does not have the correct columns: 'lower_ci' and 'upper_ci'", {
    expect_equal(length(setdiff(union(expected_colnames, given_colnames), intersect(expected_colnames, given_colnames))), 0)
  })

  # Test 4: Check for the correct number of rows (should be 1)
  test_that("Data frame does not contain the correct number of rows (should be 1)", {
    expect_equal(digest(as.integer(nrow(ci_answer5.2))), "4b5630ee914e848e8d07221556b0a2fb")
  })

  # Test 5: Check if the data values are correct using the provided digests
  test_that("The confidence interval values are not correct.", {
    # Check the lower bound
    expect_equal(digest(as.integer(ci_answer5.2$lower_ci * 10000)), 
                 'a58bfbe3bd4ac5cba3ae165e6c815640', 
                 info = "The lower_ci value is incorrect.")
    
    # Check the upper bound
    expect_equal(digest(as.integer(ci_answer5.2$upper_ci * 10000)), 
                 '7f7d209e36e1ffd64bd817f7148c90ed', 
                 info = "The upper_ci value is incorrect.")
  })
...
test_5.3 <- function() {
  ...
  # Test 1: Check if the answer object exists
  test_that('Did not assign answer to an object called "ci_answer5.3"', {
    expect_true(exists("ci_answer5.3"))
  })

  # Test 2: Check if the object is a data frame (or tibble)
  test_that("Solution should be a data frame", {
    expect_true("data.frame" %in% class(ci_answer5.3))
  })

  # Test 3: Check for the correct column names
  expected_colnames <- c("lower_ci", "upper_ci")
  given_colnames <- colnames(ci_answer5.3)
  test_that("Data frame does not have the correct columns: 'lower_ci' and 'upper_ci'", {
    expect_equal(length(setdiff(union(expected_colnames, given_colnames), intersect(expected_colnames, given_colnames))), 0)
  })
  ...
  # Test 4: Check for the correct number of rows (should be 1)
  test_that("Data frame does not contain the correct number of rows (should be 1)", {
    expect_equal(digest(as.integer(nrow(ci_answer5.3))), "4b5630ee914e848e8d07221556b0a2fb")
  })

  # Test 5: Check if the data values are correct using the provided digests
  test_that("The confidence interval values are not correct.", {
    # Check the lower bound
    expect_equal(digest(as.integer(ci_answer5.3$lower_ci * 10000)), 
                 '3da75035d6dda2eccccb671ec5de58a6', 
                 info = "The lower_ci value is incorrect.")
    
    # Check the upper bound
    expect_equal(digest(as.integer(ci_answer5.3$upper_ci * 10000)), 
                 'abd672bb8f9692d1c521c21d9e9df2a2', 
                 info = "The upper_ci value is incorrect.")
  })
