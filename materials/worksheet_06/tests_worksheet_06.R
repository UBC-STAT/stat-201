...
test_2.10 <- function() {
  ...
  test_that("Data frame does not contain the correct data", {
    expect_equal(digest(as.integer(sum(null_model$stat))), "129297e2da029ee99482f60461ecbd31")
  })
...
test_4.3A <- function() {
  test_that('Did not assign answer to an object called "sampling_dist"', {
    expect_true(exists("sampling_dist"))
  })

  test_that("Solution should be a data frame", {
    expect_true("data.frame" %in% class(sampling_dist))
  })

  expected_colnames <- c("replicate", "stat", "samp_dist")
  given_colnames <- colnames(sampling_dist)
  test_that("Data frame does not have the correct columns", {
    expect_equal(length(setdiff(
      union(expected_colnames, given_colnames),
      intersect(expected_colnames, given_colnames)
  ...
  test_that("Data frame does not contain the correct number of rows", {
    expect_equal(digest(as.integer(nrow(sampling_dist))), "f1a30baa3072c1aad822c059f35c6841")
  })

  test_that("Data frame does not contain the correct data", {
    expect_equal(digest(as.integer(sum(sampling_dist$stat) * 1000)), "e4b4d3fc067f99779fd1c3b27e21471f")
  })
