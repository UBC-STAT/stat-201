...
test_1.3 <- function(){
    ...
    test_that("Solution should be a data frame", {
          expect_true("data.frame" %in% class(answer1.3))
    })

    expected_colnames <- c("type_I_error", "type_II_error", "power_of_test")
    given_colnames <- colnames(answer1.3)
    ...
    test_that("Data frame does not contain the correct number of rows", {
          expect_equal(digest(as.integer(nrow(answer1.3))), "4b5630ee914e848e8d07221556b0a2fb")
    })

    test_that("Data frame does not contain the correct data", {
          expect_equal(digest(as.integer(sum(answer1.3$type_I_error) * 10e6)), "13dff35616f15788a3612d24ae7e23c5")
          expect_equal(digest(as.integer(sum(answer1.3$type_II_error) * 10e8)), "9e18a0fbd8fcfc89276d6d0b574a8cc6")
          expect_equal(digest(as.integer(sum(answer1.3$power_of_test) * 10e8)), "7585ead8d536129a5d6134ae78690364")
    })
...
test_3.1 <- function(){
    ...
    test_that('Solution should be "true" or "false"', {
        expect_match(answer3.1, "true|false", ignore.case = TRUE)
    })
