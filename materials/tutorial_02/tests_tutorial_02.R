...
test_2.5 <- function() {
  ...
  test_that("Data frame does not contain the correct data", {
    expect_equal(digest(sum(samples_10$geo_local_area=="Killarney")), "f0f12367a5beee1f65d2633294474dc9")
  })
  print("Success!")
}
...
test_2.6 <- function() {
  ...
  test_that("Data frame does not contain the correct data", {
    expect_equal(digest(as.integer(sum(sample_proportions_10$sample_proportion*10))), "980e5973179dc127b3ef396ecf9e942a")
  })
  print("Success!")
}
...
test_2.7 <- function() {
  ...
  test_that("Plot does not use the correct data", {
    ...
    expect_equal(digest(as.integer(sum(sampling_dist_10$data$sample_proportion)*10)), "980e5973179dc127b3ef396ecf9e942a")
  })
...
test_3.2 <- function() {
  test_that('Did not assign answer to an object called "barrier_sampling_data"', {
    expect_true(exists("barrier_sampling_data"))
  })

  test_that("barrier_sampling_data should be a data frame", {
    expect_true("data.frame" %in% class(barrier_sampling_data))
  })

  test_that("barrier_sampling_data does not have the correct columns", {
    expect_true(all(c("replicate", "p") %in% colnames(barrier_sampling_data)))
  })

  test_that("barrier_sampling_data does not contain the correct number of rows", {
    expect_equal(digest(as.integer(nrow(barrier_sampling_data))), "6e96c307060fba1b1d3a36d2410fd595")
  })

  test_that("barrier_sampling_data does not contain the correct data", {
    expect_equal(digest(round(sd(barrier_sampling_data$p), 7)), "e9db0e152223a01249a4c3225e899af9")
  })
...
test_3.5 <- function() {
  test_that('Did not assign answer to an object called "barrier_bootstrap_data"', {
    expect_true(exists("barrier_bootstrap_data"))
  })

  test_that("barrier_bootstrap_data should be a data frame", {
    expect_true("data.frame" %in% class(barrier_bootstrap_data))
  })

  test_that("barrier_bootstrap_data does not have the correct columns", {
    expect_true(all(c("replicate", "p") %in% colnames(barrier_bootstrap_data)))
  })

  test_that("barrier_bootstrap_data does not contain the correct number of rows", {
    expect_equal(digest(as.integer(nrow(barrier_bootstrap_data))), "6e96c307060fba1b1d3a36d2410fd595")
  })

  test_that("barrier_bootstrap_data does not contain the correct data", {
    expect_equal(digest(round(sum(barrier_bootstrap_data$p))), "39fe840086944a2ff92b5b0413fada63")
  })
