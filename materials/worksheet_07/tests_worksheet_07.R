...
test_2.1.1 <- function() {
  ...
  test_that('Solution should be a single character ("A", "B", "C", "D", "E", "F", "G", or "H")', {
    expect_match(answer2.1.1, "a|b|c|d|e|f|g|h", ignore.case = TRUE)
  })
...
test_2.1.4 <- function() {
  ...
  answer_as_numeric <- as.numeric(answer2.1.4)
  test_that("Solution should be a number", {
    expect_false(is.na(answer_as_numeric))
  })
...
test_2.1.5 <- function() {
  ...
  test_that('Solution should be a single character ("A", or "B")', {
    expect_match(answer2.1.5, "a|b", ignore.case = TRUE)
  })

  answer_hash <- digest(tolower(answer2.1.5))
...
test_2.1.6 <- function() {
  ...
  test_that("Solution should be the output of t.test", {
    expect_true("data.frame" %in% class(answer2.1.6))
  })

  test_that("Wrong statistic value", {
    expect_equal(digest(as.integer(answer2.1.6$statistic*10e6)), "b530578a0d50e02f4b0a2476d1d05cf6")
  })
  
  test_that("Wrong p-value", {
    expect_equal(digest(as.integer(answer2.1.6$p.value*10e6)), "529289aebf92ae5e4bf78b3453ba8d8d")
  })
  test_that("Wrong estimate", {
    expect_equal(digest(as.integer(answer2.1.6$estimate*10e6)), "2a3bad28c47da4ed94bd04dd3a8e2402")
  })
  test_that("Wrong parameter", {
    expect_equal(digest(answer2.1.6$parameter), "63996d1e8f7bb83b447df0b0188f621f")
  })
  test_that("Wrong alternative hypothesis", {
    expect_equal(digest(answer2.1.6$alternative), "d095e00cba6f80bf1d5ad5db0400f812")
  })
...
test_2.2.6 <- function() {
  ...
  test_that("Wrong confidence interval (check conf.level)", {
    expect_equal(digest(as.integer(answer2.2.6$conf.low*10e6)), "f6fd55f3e9f4b693bb706c2dd4b149c7")
    expect_equal(digest(as.integer(answer2.2.6$conf.high*10e6)), "e70c8aebf8e11c6fa4c9d6fc367d7fc9")
  })
...
test_2.5.2 <- function() {
  test_that('Did not assign answer to an object called "answer2.5.2"', {
    expect_true(exists("answer2.5.2"))
  })

  test_that('Solution should be a single character ("A", "B", "C", or "D")', {
    expect_match(answer2.5.2, "a|b|c|d", ignore.case = TRUE)
  })

  answer_hash <- digest(tolower(answer2.5.2))
...
test_2.5.3 <- function() {
  ...
  answer_as_numeric <- as.numeric(answer2.5.3)
  test_that("Solution should be a number", {
    expect_false(is.na(answer_as_numeric))
  })
...
test_2.5.5 <- function() {
  ...
  test_that('Solution should be a single character ("A", "B")', {
    expect_match(answer2.5.5, "a|b", ignore.case = TRUE)
  })

  answer_hash <- digest(tolower(answer2.5.5))
...
test_2.5.6 <- function() {
  ...
  test_that("Solution should be the output of t.test", {
    expect_true("data.frame" %in% class(answer2.5.6))
  })

  test_that("Wrong statistic value", {
    expect_equal(digest(as.integer(answer2.5.6$statistic*10e6)), "5f3f9a2b249700c4abdf83a429cc4b8f")
  })
  
  test_that("Wrong p-value", {
    expect_equal(digest(as.integer(answer2.5.6$p.value*10e6)), "1473d70e5646a26de3c52aa1abd85b1f")
  })
  test_that("Wrong estimate", {
    expect_equal(digest(as.integer(answer2.5.6$estimate*10e6)), "a463bf6ff963e84808b4fc7072ce750d")
  })
  test_that("Wrong parameter", {
    expect_equal(digest(answer2.5.6$parameter), "cb296d017f50125cb5c4cd8e507aaa1b")
  })
  test_that("Wrong alternative hypothesis", {
    expect_equal(digest(answer2.5.6$alternative), "0ff92e9c2aed54e21f95489292c00267")
  })
