...
test_2.1.1 <- function() {
  ...
  test_that("Solution should be a ggplot object", {
    expect_true(is.ggplot(answer2.1.1))
  })

  properties <- c(answer2.1.1$layers[[1]]$mapping, answer2.1.1$mapping)
  ...
  test_that("Plot does not have the correct layers", {
    expect_true("GeomBar" %in% class(answer2.1.1$layers[[1]]$geom))
  ...
  expect_equal(
    digest(as.integer(mget("stat_params", answer2.1.1$layers[[1]])[["stat_params"]][["binwidth"]]*100)),
    "be3c152f6f6bcd5f85f9e4cba49b1e48"
    )
  })

  test_that("Plot does not use the correct data", {
    expect_equal(digest(nrow(answer2.1.1$data)), "bbf77ebc365ea5855142ed8da3c87cf6")
    expect_equal(digest(round(sum(answer2.1.1$data$body_temp))), "5c9b553f50867cee815aa458af1d3b42")
  })

  test_that("x-axis label should be descriptive and human readable", {
    expect_false(answer2.1.1$labels$x == toString(rlang::get_expr(properties$x)))
  })

  test_that("Plot should have a title", {
    expect_true("title" %in% names(answer2.1.1$labels))
  })
...
test_2.1.4 <- function() {
  test_that('Did not assign answer to an object called "null_model2.1.4"', {
    expect_true(exists("null_model2.1.4"))
  })

  test_that("Solution should be a ggplot object", {
    expect_true(is.ggplot(null_model2.1.4))
  })

  properties <- c(null_model2.1.4$layers[[1]]$mapping, null_model2.1.4$mapping)
  ...
  test_that("Plot does not have the correct layers", {
    expect_true("GeomLine" %in% class(null_model2.1.4$layers[[1]]$geom))

    # Remove if not needed:
    expect_true("GeomSegment" %in% class(null_model2.1.4$layers[[2]]$geom))
    # expect_true("GeomVline" %in% class(steam_ci_plot$layers[[2]]$geom))
  })


  test_that("Plot does not use the correct data", {
    expect_equal(digest(nrow(null_model2.1.4$data)), "c806da0181d245ba4f35cf385ab29983")
    expect_equal(digest(round(sum(null_model2.1.4$data$t))), "908d1fd10b357ed0ceaaec823abf81bc")
  })

  test_that("x-axis label should be descriptive and human readable", {
    expect_false(null_model2.1.4$labels$x == toString(rlang::get_expr(properties$x)))
  })

  test_that("Plot should have a title", {
    expect_true("title" %in% names(null_model2.1.4$labels))
  })
