...
test_3.1 <- function() {
  ...
  test_that('Did not assign answer to an object called "answer3.1_std_dev"', {
    expect_true(exists("answer3.1_std_error"))
  })

  answer_mean_as_numeric <- as.numeric(answer3.1_mean)
  answer_sd_as_numeric <- as.numeric(answer3.1_std_error)
  test_that("Solution is incorrect", {
...
test_3.4 <- function() {
  ...
  test_that("Data frame does not contain the correct data", {
    expect_equal(digest(as.integer(sum(mean_body_mass_chinstrap_ci$lower_ci) * 1000)), "0cda3dc7e48bd7032907218ae9ac0b96")
    expect_equal(digest(as.integer(sum(mean_body_mass_chinstrap_ci$upper_ci) * 1000)), "3921f6b30507db9577342df57433f505")
  })
...
test_3.5 <- function() {
  ...
  test_that('Did not assign answer to an object called "answer3.5_std_dev"', {
    expect_true(exists("answer3.5_std_error"))
  })

  answer_mean_as_numeric <- as.numeric(answer3.5_mean)
  answer_sd_as_numeric <- as.numeric(answer3.5_std_error)
  test_that("Solution is incorrect", {
