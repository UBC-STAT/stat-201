...
test_6.0 <- function() {
  ...
  test_that("Plot does not have the correct bin width", {
    expect_equal(
      digest(as.integer(round(mget("stat_params", sampling_dist_30$layers[[1]])[["stat_params"]][["binwidth"]]*30))),
  ...
  test_that("Plot does not use the correct data", {
    ...
    expect_equal(digest(as.integer(round(sum(sampling_dist_30$data$sample_proportion)*30))), "22ccbb1d89a68e2c01565574d0bdd3f6")
  })
...
test_6.1 <- function() {
  ...
  test_that("Plot does not have the correct bin width", {
    expect_equal(
      digest(as.integer(round(mget("stat_params", sampling_dist_100$layers[[1]])[["stat_params"]][["binwidth"]]*100))),
      "4b5630ee914e848e8d07221556b0a2fb"
    )
  })
  ...
  test_that("Plot does not use the correct data", {
    ...
    expect_equal(digest(as.integer(round(sum(sampling_dist_100$data$sample_proportion)*100))), "8910cf9b125a2c8164816bbf8593228a")
  })
...
test_6.4 <- function() {
  ...
  test_that("Data frame does not contain the correct data", {
    expect_equal(digest(sample_10$geo_local_area), "5f861fd1b453d612819134d9fdee416c")
  })
  print("Success!")
}
...
test_6.5 <- function() {
  ...
  test_that("Data frame does not contain the correct data", {
    expect_equal(digest(as.integer(sum(resampled_proportions_10$sample_proportion))), "9d3eea063e778ae48c064694635894a9")
  })
...
test_6.6 <- function() {
  ...
  test_that("Solution is incorrect", {
    expect_equal(digest(as.integer(answer_mean_as_numeric * 100)), "16071ab8270571c6c83d682892e00ea5")
    expect_equal(digest(as.integer(answer_sd_as_numeric * 100)), "e03a89536262b6a0e2beabd90a841c43")
  })
