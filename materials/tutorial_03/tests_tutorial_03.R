...
test_2.6 <- function() {
  test_that('Did not assign answer to an object called "sampling_dist_150x2000"', {
    expect_true(exists("sampling_dist_150x2000"))
  })
  properties <- c(sampling_dist_150x2000$layers[[1]]$mapping, sampling_dist_150x2000$mapping)
  ...
  test_that("Plot is not the correct type", {
    expect_true("GeomBar" %in% class(sampling_dist_150x2000$layers[[1]]$geom))
  })

  test_that("Plot does not have the correct bin width", {
  expect_equal(
    digest(as.integer(mget("stat_params", sampling_dist_150x2000$layers[[1]])[["stat_params"]][["binwidth"]])),
    "7d2842cab7725fd8f382293e410d42b2"
    )
  })

  test_that("Plot does not use the correct data", {
    expect_equal(digest(nrow(sampling_dist_150x2000$data)), "6e96c307060fba1b1d3a36d2410fd595")
    expect_equal(digest(round(sum(sampling_dist_150x2000$data$sample_var) * 100000)), "ed5451e352b73df743bf147e6fdacfc8")
  })

  test_that("x-axis label should be descriptive and human readable", {
    expect_false(sampling_dist_150x2000$labels$x == "sample_var")
  })

  test_that("Plot should have a title", {
    expect_true("title" %in% names(sampling_dist_150x2000$labels))
  })
...
test_3.6 <- function() {
  ...
  test_that('Solution should be a single character ("A", "B", "C", "D", "E", "F", "G", or "H")', {
    expect_match(answer3.6, "a|b|c|d|e|f|g|h", ignore.case = TRUE)
  })
...
test_3.7 <- function() {
  ...
  answer_hash <- digest(tolower(answer3.7))
  if (answer_hash == "d2a90307aac5ae8d0ef58e2fe730d38b") {
...
test_6.6 <- function() {
  ...
  test_that("Plot does not use the correct data", {
    ...
    expect_equal(digest(round(as.integer(bootstrap_dist_10$data$sample_proportion))), "9d14e1ec7efac700c2e36d8121b17af7")
  })

  test_that("x-axis label should be descriptive and human readable", {
    expect_false(bootstrap_dist_10$labels$x == "sample_proportion")
  })
...
test_6.7 <- function() {
  test_that('Did not assign answer to an object called "answer6.7_mean"', {
    expect_true(exists("answer6.7_mean"))
  })
  test_that('Did not assign answer to an object called "answer6.7_std_dev"', {
    expect_true(exists("answer6.7_std_error"))
  })

  answer_mean_as_numeric <- as.numeric(answer6.7_mean)
  answer_sd_as_numeric <- as.numeric(answer6.7_std_error)
  test_that("Solution is incorrect", {
    expect_equal(digest(as.integer(answer_mean_as_numeric * 100)), "be3c152f6f6bcd5f85f9e4cba49b1e48")
    expect_equal(digest(as.integer(answer_sd_as_numeric * 100)), "16071ab8270571c6c83d682892e00ea5")
  })
...
test_6.8 <- function() {
  test_that('Did not assign answer to an object called "answer6.8"', {
    expect_true(exists("answer6.8"))
  })

  test_that('Solution should be "true" or "false"', {
    expect_match(answer6.8, "true|false", ignore.case = TRUE)
  })

  answer_hash <- digest(tolower(answer6.8))
