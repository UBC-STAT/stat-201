...
test_1.0 <- function() {
  ...
  test_that('Solution should be a string of ALL the following characters ("A", "B", "C", and "D")', {
    expect_match(answer1.0, "^[aA|bB|cC|dD]{4}$")
  })

  test_that("Solution is incorrect", {
    expect_equal(digest(tolower(answer1.0)), "3abf323ffd537832dea46271ef88d4a3")
  })
...
test_2.3 <- function() {
  ...
  test_that("Solution is incorrect", {
    expect_equal(digest(as.integer(round(answer_as_numeric * 1000000))), "9cb94af2f4e658222cf15337c6fd41dc")
  })
...
test_2.5 <- function() {
  ...
  test_that("Solution is incorrect", {
    expect_equal(digest(as.integer(round(answer_as_numeric * 1000000))), "31fd6a22f6993fe7f81afdbef51e8141")
  })
...
test_3.2 <- function() {
  ...
  test_that("Data frame does not contain the correct data", {
    expect_equal(digest(as.integer(round(ci$ci_lower * 1000000))), "77ef7200ca25d3231daa2f6858555305")
    expect_equal(digest(as.integer(round(ci$ci_upper * 1000000))), "f2f62be2160a51c069248ca724d0e01a")
  })
...
test_3.7 <- function() {
  test_that('Did not assign answer to an object called "sampling_dist_estimate_sample_id_1"', {
    expect_true(exists("sampling_dist_estimate_sample_id_1"))
  })

  test_that("Solution should be a data frame", {
    expect_true("data.frame" %in% class(sampling_dist_estimate_sample_id_1))
  })

  expected_colnames <- c("replicate", "bootstrap_mean")
  given_colnames <- colnames(sampling_dist_estimate_sample_id_1)
  test_that("Data frame does not have the correct columns", {
    expect_equal(length(setdiff(
      union(expected_colnames, given_colnames),
      intersect(expected_colnames, given_colnames)
  ...
  test_that("Data frame does not contain the correct number of rows", {
    expect_equal(digest(as.integer(nrow(sampling_dist_estimate_sample_id_1))), "b6a6227038bf9be67533a45a6511cc7e")
  })

  test_that("Data frame does not contain the correct data", {
    expect_equal(digest(as.integer(sum(sampling_dist_estimate_sample_id_1$bootstrap_mean) * 1000000)), "c7f66da1cae4f223b9bae717f05900f7")
  })
...
test_4.0 <- function() {
  ...
  if (answer_hash == "05ca18b596514af73f6880309a21b5dd") {
    print("This is a common misinterpretation of confidence intervals. Once an interval is calculated, it either captures or does not capture the true parameter (i.e., the probability of the interval capturing the true parameter is either 0 or 1)")
  }
