...
test_2.9 <- function() {
  ...
  if (answer_hash == "127a2ec00989b9f7faf671ed470be7f8") {
    print("If 25% of the observations fall below lower_quantile and 75% fall above upper_quantile, what proportion lie between the two?")
...
test_3.8 <- function() {
  ...
  test_that("Data frame does not contain the correct data", {
    expect_equal(digest(as.integer(sum(intervals$ci_lower) * 1000000)), "5e2e6071008dd2bd800537489b3a59c5")
    expect_equal(digest(as.integer(sum(intervals$ci_upper) * 1000000)), "c7f66da1cae4f223b9bae717f05900f7")
  })
