...
test_1.3 <- function() {
  ...
  test_that('Did not assign answer to an object called "variancev"', {
    expect_true(exists("variance"))
  })
  test_that('Did not assign answer to an object called "correlation"', {
    expect_true(exists("correlation"))
  })

  answers <- c(mean, proportion, median, variance, correlation) %>%
    map(as.numeric)
  ...
  test_that("At least one term-definition match is incorrect", {
    expect_equal(digest(as.numeric(mean)), "5e338704a8e069ebd8b38ca71991cf94")
    expect_equal(digest(as.numeric(proportion)), "6717f2823d3202449301145073ab8719")
    expect_equal(digest(as.numeric(median)), "e5b57f323c7b3719bbaaf9f96b260d39")
    expect_equal(digest(as.numeric(variance)), "db8e490a925a60e62212cefc7674ca02")
    expect_equal(digest(as.numeric(correlation)), "dbc09cba9fe2583fb01d63c70e1555a8")
  })
...
test_1.4 <- function() {
  ...
  test_that('Did not assign answer to an object called "random_sampling"', {
    expect_true(exists("random_sampling"))
  })
  test_that('Did not assign answer to an object called "representative_sampling"', {
    expect_true(exists("representative_sampling"))
  })
  ...
  test_that("At least one term-definition match is incorrect", {
    expect_equal(digest(as.numeric(point_estimate)), "0aee9b78301d7ec8998971363be87c03")
    expect_equal(digest(as.numeric(population)), "6717f2823d3202449301145073ab8719")
    expect_equal(digest(as.numeric(random_sampling)), "db8e490a925a60e62212cefc7674ca02")
    expect_equal(digest(as.numeric(representative_sampling)), "90a7653d717dc1553ee564aa27b749b9")
    expect_equal(digest(as.numeric(population_parameter)), "e5b57f323c7b3719bbaaf9f96b260d39")
    expect_equal(digest(as.numeric(sample)), "5e338704a8e069ebd8b38ca71991cf94")
    expect_equal(digest(as.numeric(observation)), "4a5d7d50676e6d0ea065f445d8a5539d")
    expect_equal(digest(as.numeric(sampling_distribution)), "dbc09cba9fe2583fb01d63c70e1555a8")
  })
...
test_2.9 <- function() {
  ...
  test_that('Solution should be a single character ("A", "B", or "C")', {
    expect_match(answer2.9, "a|b|c", ignore.case = TRUE)
  })

  answer <- digest(tolower(answer2.9))
  if (answer == "127a2ec00989b9f7faf671ed470be7f8") {
    print("It is very unlikely that we choose the same set of individuals from the population across multiple samples.")
  } else if (answer == "ddf100612805359cd81fdc5ce3b9fbba") {
    print("It is very unlikely that we choose the same set of individuals from the population across multiple samples.")
  }
  
  test_that("Solution is incorrect", {
    expect_equal(digest(tolower(answer2.9)), "6e7a8c1c098e8817e3df3fd1b21149d1")
  })
